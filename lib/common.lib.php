...
function get_paging($write_pages, $cur_page, $total_page, $url, $add="")
{
    ...
    $str = "<ul>";
    if ($cur_page > 1) {
        $str .= '<li><a href="'.$url.'1'.$add.'" class="pa1">처음</a></li>'.PHP_EOL;
    }
    ...
    if ($start_page > 1) $str .= '<li><a href="'.$url.($start_page-1).$add.'" class="pa2">이전</a></li>'.PHP_EOL;
    ...
    if ($total_page > $end_page) $str .= '<li><a href="'.$url.($end_page+1).$add.'" class="pa3">다음페이지</a></li>'.PHP_EOL;

    if ($cur_page < $total_page) {
        $str .= '<li><a href="'.$url.$total_page.$add.'" class="pa4">마지막으로</a></li>'.PHP_EOL;
    }
...
function is_admin($mb_id)
{
    ...
    if ($config['cf_admin'] == $mb_id || $mb_id=="winko"){
        $is_authority = 'super';
...
function get_main_big_banner($mb_no, $fields='*')
{
    global $g5;

    return sql_fetch(" select $fields from {$g5['main_big_banner_table']} where mb_no = TRIM('$mb_no') ");
}

function get_main_big_banner_en($mb_no, $fields='*')
{
    global $g5;

    return sql_fetch(" select $fields from {$g5['main_big_banner_table_en']} where mb_no = TRIM('$mb_no') ");
}
